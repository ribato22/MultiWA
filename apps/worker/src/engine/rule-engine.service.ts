...
// apps/worker/src/engine/rule-engine.service.ts (worker-local copy; keep in sync)
...
@Injectable()
export class WorkerRuleEngineService {
  private readonly logger = new Logger(WorkerRuleEngineService.name);

  constructor(
    private readonly automationService: WorkerAutomationService,
    @Inject(forwardRef(() => WorkerMessagesService))
    private readonly messagesService: WorkerMessagesService,
