...
import { evaluateColdCircuit, applyAckStatusUpdate, handleAutoRejectCall, resolveEngineType as resolveEngineTypeShared, handleInboundMessage as handleInboundMessageShared } from '@multiwa/engine-runtime';
...
export class EngineManagerService implements OnModuleDestroy, OnModuleInit {
  ...
  async connectProfile(profileId: string): Promise<{ status: string; message: string }> {
    ...
    const engineConfig: EngineConfig = {
      ...
      // Inbound handling is shared with the API engine-manager via
      // @multiwa/engine-runtime so both processes run one implementation.
      onMessage: async (message: any) =>
        handleInboundMessageShared(message, profileId, {
          logger: this.logger,
          ...
          processAutomation: (msg) => this.ruleEngine.processMessage(msg),
